...
use opencv::prelude::*;
use shaoooh::vision::BishaanVisionSocket;
use std::sync::Arc;
...
async fn main() {
    ...
    let (error_tx_c, _error_rx) = broadcast::channel(32);
    ...
    tokio::spawn(async move {
        ...
        let _vision_handle = tokio::spawn(vision.task());
    });
    ...
    loop {
        tokio::select! {
            _ = t_frame_rx.changed() => {
                ...
                opencv::imgcodecs::imwrite(&top_path, &top, &opencv::core::Vector::new()).expect("Failed to write iamge");
            ...
            _ = b_frame_rx.changed() => {
                ...
                opencv::imgcodecs::imwrite(&bot_path, &bot, &opencv::core::Vector::new()).expect("Failed to write iamge");
